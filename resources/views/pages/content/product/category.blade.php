...
@section('content')
   <div class="m-6 ">
      <div class="relative flex flex-row justify-end rounded gap-6 bg-white p-3 shadow-sm  ring-1 ring-gray-900/5 ">
         ...
         <a href="{{route('showCategory')}}" class="flex justify-end my-auto ">
...
<div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
  <table class="w-full bg-white text-left text-sm text-gray-500">
    <thead class="bg-gray-50">
      <tr>
        <th scope="col" class="px-6 py-4 font-medium text-gray-900 min-w-[150px]">Category Name</th>
        <th scope="col" class="px-6 py-4 font-medium text-gray-900 min-w-[50px]"></th>
      </tr>
    </thead>
    <tbody class=" divide-gray-100 border-t border-gray-100">
         @if (request()->routeIs('showCategory'))
            @foreach ($categories as $category)
                  <tr class="hover:bg-gray-50">
                  <td class="min-w-[150px] px-6 py-4 font-medium text-gray-700 ">{{ $category->name }}</td>

                  <td class="px-6 py-4">
                     <div class="flex justify-end gap-4">
                        <form action="{{ route('deleteCategory', $category->uuid) }}" method="post">
                           @csrf
                           @method('DELETE')
                           <button type="submit">
                        ...
                        <a href="{{route('categoryEdit',$category->uuid)}}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6" x-tooltip="tooltip" >
...
<!-- component -->
<div id='modal-container-category' class=" {{request()->routeIs('showCategory') ? 'hidden' : '' }} flex items-center justify-center max-w-screen max-h-screen ml-[-25%]">
   <div x-show="showModal" class="fixed inset-0 transition-opacity z-20">
      <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
   </div>
   <a id="close-modal-category"
      @if(request()->routeIs('categoryEdit'))
   ...
   <div class=" text-center top-8 fixed z-20 ">
      <div class="py-16 px-7 rounded-md bg-white">
         <form class=""  method="POST" action="{{request()->routeIs('showCategory') ? route('createCategory') : route('categoryUpdate',$category->uuid)}}">
            @csrf
            @if (request()->routeIs('categoryEdit'))
               @method('PUT')
            @endif
            <div class="grid md:grid-cols-2 grid-cols-1 gap-4">
               <div class="md:col-span-2">
                  <input type="text" id="category" name="category" placeholder=" {{request()->routeIs('showCategory') ? 'Product Category' : $category->name}} " value="{{request()->routeIs('showCategory') ? '' : $category->name}}" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">
               </div>
               <div class="md:col-span-2">
...
<script>
document.getElementById('close-modal-category').addEventListener('click', function() { 
   document.getElementById('modal-container-category').classList.add('hidden');
})
document.getElementById('show-modal').addEventListener('click', function() { 
   document.getElementById('modal-container-category').classList.remove('hidden');
})
